...
local function main()
    ...
    if ( legIN:ready() ) then
        ...
        -- logging
        logify('module', 'callcontrol', 'space', 'callctl', 'action', 'inbounnd_call_request' , 'uuid', in_uuid, 
...
if not result then
    logify("module=callcontrol, space=callctl,  action=exception, error="..tostring(error))
end
---- close log ----
syslog.closelog()